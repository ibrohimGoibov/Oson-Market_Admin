...
const Layout = () => {
  ...
  return (
    <div className={`min-h-screen ${theme === 'dark' ? 'bg-gray-800 text-white' : 'bg-gray-100 text-black'}`}>
        ...
        <div className="flex-1 mx-10">
          <div className="relative max-w-[420px] mx-auto">
            <input
              type="text"
              placeholder="Search..."
              className="w-full pl-10 pr-4 py-2 rounded-lg border border-blue-500 outline-none focus:ring-2 focus:ring-blue-500"
            />
      ...
      <div className="flex">
        <aside className={`w-[280px] p-6 shadow-xl ${theme === 'dark' ? 'bg-gray-800' : 'bg-white'}`}>
          <h2 className="text-2xl font-bold text-blue-600 mb-10 text-center">Dashboard</h2>
        ...
        <main className="flex-1 p-8">
          <div className={`rounded-2xl shadow-md p-6 min-h-[90vh] ${theme === 'dark' ? 'bg-gray-800 text-white' : 'bg-white text-black'}`}>
            <Outlet />
          </div>
        </main>
      </div>
